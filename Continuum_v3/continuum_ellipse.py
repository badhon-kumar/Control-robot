...
@dataclass
class ModifiedPCCParams:
    """
    Optional calibration layer on top of the paper PCC model.
    Identity values reproduce the original Zhai-style PCC exactly.
    """
    enabled: bool = False
    length_scale: Tuple[float, float, float] = (1.0, 1.0, 1.0)
    radius_scale: Tuple[float, float, float] = (1.0, 1.0, 1.0)
    curvature_gain: Tuple[float, float, float] = (1.0, 1.0, 1.0)
    deadzone_m: Tuple[float, float, float] = (0.0, 0.0, 0.0)
    pull_gain: Tuple[float, float, float] = (1.0, 1.0, 1.0)
    release_gain: Tuple[float, float, float] = (1.0, 1.0, 1.0)
...
def _apply_modified_pcc(u: np.ndarray,
                        L: List[float],
                        r: List[float],
                        mod: Optional[ModifiedPCCParams]):
    """Return calibrated (u, L, r, curvature_gain) for modified PCC."""
    if mod is None or not mod.enabled:
        return u, list(L), list(r), np.ones(3)

    u_eff = np.array(u, dtype=float).copy()
    for i in range(3):
        sgn = 1.0 if u_eff[i] >= 0.0 else -1.0
        mag = max(0.0, abs(u_eff[i]) - max(0.0, float(mod.deadzone_m[i])))
        gain = float(mod.pull_gain[i]) if sgn >= 0.0 else float(mod.release_gain[i])
        u_eff[i] = sgn * mag * max(0.0, gain)

    L_eff = [max(1e-6, float(L[i]) * max(0.05, float(mod.length_scale[i])))
             for i in range(3)]
    r_eff = [max(1e-6, float(r[i]) * max(0.05, float(mod.radius_scale[i])))
             for i in range(3)]
    k_gain = np.array([max(0.01, float(v)) for v in mod.curvature_gain])
    return u_eff, L_eff, r_eff, k_gain


def forward_kinematics(u: np.ndarray,
                        L: List[float] = L_SEG,
                        r: List[float] = R_TENDON,
                        mod: Optional[ModifiedPCCParams] = None) -> np.ndarray:
    """
    PCC forward kinematics (Eqs. 3–5).
    u = [Δl₁, Δl₂, Δl₃] in metres.
    Returns [x, y, ψ] in metres and radians.
    """
    u_eff, L_eff, r_eff, k_gain = _apply_modified_pcc(u, L, r, mod)
    th = pcc_angles(u_eff, r_eff) * k_gain
    x, y = 0.0, 0.0
...
def model_jacobian(u: np.ndarray,
    ...
    for j in range(3):
        u_p = u.copy(); u_p[j] += h
        u_m = u.copy(); u_m[j] -= h
        mJ[:, j] = (forward_kinematics(u_p, L, r, mod) -
                    forward_kinematics(u_m, L, r, mod)) / (2 * h)
    return mJ
...
class KalmanJacobianController:
    ...
    def __init__(self,
                 ...
                 modified_pcc: Optional[ModifiedPCCParams] = None):
        ...
        self.modified_pcc = modified_pcc or ModifiedPCCParams()
        ...
        self.cJ_prev = model_jacobian(np.zeros(3), self.L, self.r, self.modified_pcc)
    ...
    def _fk(self, u): return forward_kinematics(u, self.L, self.r, self.modified_pcc)
    def _mj(self, u): return model_jacobian(u, self.L, self.r, self.modified_pcc)
...
class TrajectoryControlPanel(tk.Frame):
    ...
    def __init__(self, parent, get_robot, set_disps_fn, log_fn, simulated=True,
        ...
        self._mpcc_var  = tk.BooleanVar(value=False)
        self._mpcc_len_var = tk.StringVar(value="1,1,1")
        self._mpcc_rad_var = tk.StringVar(value="1,1,1")
        self._mpcc_curv_var = tk.StringVar(value="1,1,1")
        self._mpcc_deadzone_var = tk.StringVar(value="0,0,0")
        self._mpcc_pull_var = tk.StringVar(value="1,1,1")
        self._mpcc_release_var = tk.StringVar(value="1,1,1")
    ...
    def _build(self):
        ...
        mpcc_row0 = len(params) + 2
        tk.Checkbutton(cc, text="Use modified PCC calibration",
                       variable=self._mpcc_var, font=(FNT,11,"bold"),
                       fg=BLUE, bg=CARD, selectcolor=INPUT, activebackground=CARD).grid(
            row=mpcc_row0, column=0, columnspan=2, sticky="w", padx=8, pady=(8,2))
        tk.Label(cc, text="Triples are segment 1,2,3. Identity values keep paper PCC.",
                 font=(FNT,9), fg=MUTED, bg=CARD).grid(
            row=mpcc_row0+1, column=0, columnspan=2, sticky="w", padx=8, pady=(0,3))
        mpcc_params = [
            ("L scale:",       self._mpcc_len_var),
            ("r scale:",       self._mpcc_rad_var),
            ("curv gain:",     self._mpcc_curv_var),
            ("deadzone mm:",   self._mpcc_deadzone_var),
            ("pull gain:",     self._mpcc_pull_var),
            ("release gain:",  self._mpcc_release_var),
        ]
        for r,(lbl,var) in enumerate(mpcc_params, mpcc_row0+2):
            tk.Label(cc, text=lbl, font=(FNT,10), fg=MUTED, bg=CARD).grid(
                row=r, column=0, sticky="w", padx=8, pady=2)
            tk.Entry(cc, textvariable=var, width=14, font=(FNT,10), bg=INPUT, fg=TEXT,
                     insertbackground=TEXT, relief="flat",
                     highlightbackground=BORDER, highlightthickness=1).grid(
        ...
        # Error plot
        tk.Label(err_frame, text="TRACKING ERROR COMPONENTS  (mm)",
    ...
    def _get_triple(self, var: tk.StringVar, default: Tuple[float, float, float]) -> Tuple[float, float, float]:
        try:
            parts = [p.strip() for p in var.get().replace(";", ",").split(",") if p.strip()]
    ...
    def _make_modified_pcc(self) -> ModifiedPCCParams:
        deadzone_mm = self._get_triple(self._mpcc_deadzone_var, (0.0, 0.0, 0.0))
        return ModifiedPCCParams(
            enabled=bool(self._mpcc_var.get()),
            length_scale=self._get_triple(self._mpcc_len_var, (1.0, 1.0, 1.0)),
            radius_scale=self._get_triple(self._mpcc_rad_var, (1.0, 1.0, 1.0)),
            curvature_gain=self._get_triple(self._mpcc_curv_var, (1.0, 1.0, 1.0)),
            deadzone_m=tuple(max(0.0, v) / 1000.0 for v in deadzone_mm),
            pull_gain=self._get_triple(self._mpcc_pull_var, (1.0, 1.0, 1.0)),
    ...
    def _control_loop(self):
        ...
        while not self._abort_flag.is_set() and t <= total_dur:
            ...
            self._err_x_hist.append(err[0]*1000)
            self._err_y_hist.append(err[1]*1000)
    ...
    def _redraw_err(self):
        ...
        if not self._err_x_hist or not self._err_y_hist:
            cv.create_text(w//2, h//2, text="X/Y error plots will appear here",
        ...
        margin_y = 12
        gap = 8
        plot_h = max(24, (h - 2 * margin_y - gap) / 2.0)
        panels = [
            (margin_y, margin_y + plot_h, self._err_x_hist, "x error", BLUE),
            (margin_y + plot_h + gap, margin_y + 2 * plot_h + gap, self._err_y_hist, "y error", RED),
        ]
        N = min(len(self._err_x_hist), len(self._err_y_hist))
        x_right = w - 10

        for x in range(int(margin_x), w, 40):
            cv.create_line(x, 0, x, h, fill="#d8dfe8", width=1)

        for y0, y1, hist, label, color in panels:
            values = hist[-N:]
            max_abs = max(max(abs(v) for v in values), 1.0)
            y_mid = (y0 + y1) / 2.0
            cv.create_rectangle(margin_x, y0, x_right, y1, outline="#c9d2dd", width=1)
            cv.create_line(margin_x, y_mid, x_right, y_mid, fill="#8b98aa", width=1)
            cv.create_text(4, y0 + 4, text=label, fill=color,
                           font=(FNT,8,"bold"), anchor="nw")
            cv.create_text(w-4, y0 + 2, text=f"+{max_abs:.1f}",
                           fill=DIM, font=(FNT,8), anchor="ne")
            cv.create_text(w-4, y1 - 2, text=f"-{max_abs:.1f}",
                           fill=DIM, font=(FNT,8), anchor="se")

            def to_px(i, e):
                px_ = margin_x + (i / max(N - 1, 1)) * (x_right - margin_x)
                py_ = y_mid - (e / max_abs) * ((y1 - y0) * 0.42)
                return px_, py_

            if N >= 2:
                pts = [to_px(i, e) for i, e in enumerate(values)]
                flat = [v for p in pts for v in p]
                cv.create_line(*flat, fill=color, width=1.5, smooth=True)
