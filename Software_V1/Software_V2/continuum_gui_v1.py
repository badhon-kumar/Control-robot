...
Because distal tendons pass through proximal disks, segment angles are coupled
by the PCC actuation equations; a commanded pair still pulls/releases only its
own two motors.
The actuation input for each segment is the tendon length differential:
Active code note: motor pair commands are converted to Delta l_i with a half
signed pair difference, then theta_1..theta_3 are solved with Eqs. 7-9.
...
DISK_DIAMETER_MM  = 13.0                     # paper: manipulator diameter
SPACER_DISKS_PER_SEG = 5                     # paper: 5 spacer disks / segment
END_DISKS_PER_SEG    = 1                     # paper: 1 tendon end disk / segment
...
SEG_COLORS  = ["#f5a623", "#ff6b35", "#0087c9"]   # orange, red-orange, blue
...
def disk_layout_mm():
    """Return base/spacer/end disk positions along the manipulator arc."""
    disks = [(0.0, -1, "base")]
    base_s = 0.0
    for seg, seg_len in enumerate(SEG_LENGTHS_MM):
        spacing = seg_len / (SPACER_DISKS_PER_SEG + END_DISKS_PER_SEG)
        for n in range(1, SPACER_DISKS_PER_SEG + END_DISKS_PER_SEG + 1):
            kind = "end" if n == SPACER_DISKS_PER_SEG + END_DISKS_PER_SEG else "spacer"
            disks.append((base_s + n * spacing, seg, kind))
        base_s += seg_len
    return disks
...
def compute_distributed_kinematics_legacy(disps_mm: List[float]) -> dict:
    """
...
def motor_disps_to_paper_deltas(disps_mm: List[float]) -> List[float]:
    """
    Convert signed antagonistic motor positions to the paper's Delta l inputs.

    The GUI stores both motors in a pair: one pulls by +d while its mate
    releases by -d. The paper's Delta l_i is the one-side tendon change, so
    (+d - -d) must be halved back to d.
    """
    return [
        0.5 * (disps_mm[0] - disps_mm[1]),
        0.5 * (disps_mm[2] - disps_mm[3]),
        0.5 * (disps_mm[4] - disps_mm[5]),
    ]


def pcc_angles_from_deltas(deltas_mm: List[float]) -> List[float]:
    """
    Paper PCC actuation model, Eqs. 7-9.

    theta_1 = Delta l_1 / r_1
    theta_2 = (Delta l_2 - r_2 theta_1) / r_1
    theta_3 = (Delta l_3 - r_3 theta_1 - r_2 theta_2) / r_1
    """
    r1, r2, r3 = TENDON_OFFSETS_MM
    dl1, dl2, dl3 = deltas_mm

    theta1 = dl1 / r1 if r1 else 0.0
    theta2 = (dl2 - r2 * theta1) / r1 if r1 else 0.0
    theta3 = (dl3 - r3 * theta1 - r2 * theta2) / r1 if r1 else 0.0

    # Display/hardware safety clamp; the equation itself is the unclamped value.
    lim = math.radians(150)
    return [max(-lim, min(lim, th)) for th in (theta1, theta2, theta3)]


def compute_pcc_kinematics(disps_mm: List[float]) -> dict:
    """
    Piecewise constant curvature forward kinematics from the paper.

    The 6 motor positions are first reduced to three antagonistic tendon
    inputs Delta l_i, then Eqs. 7-9 solve segment bending angles. The body is
    integrated with constant curvature theta_i / L_i on each segment, matching
    the PCC transform in Eqs. 3-5 while keeping the existing canvas point data.
    """
    N_PTS = 120
    L_tot = TOTAL_LENGTH_MM
    ds = L_tot / N_PTS
    ...
    actuation_deltas = motor_disps_to_paper_deltas(disps_mm)
    thetas = pcc_angles_from_deltas(actuation_deltas)
    seg_curvatures = [
        thetas[i] / SEG_LENGTHS_MM[i] if SEG_LENGTHS_MM[i] else 0.0
        for i in range(3)
    ]
    ...
    return {
        ...
        "end_pts": end_pts,
        "thetas": thetas,
        "actuation_deltas": actuation_deltas,
        "tip_x": tip_x,
        "tip_y": tip_y,
        "psi": psi,
        # legacy keys so HUD / top-view code that references seg_points still works
        ...
        with open(path, newline="", encoding="utf-8-sig") as f:
            ...
            for row_num, row in enumerate(reader, start=2):
                ...
                for m in range(1, 7):
                    ...
                    disp = max(0.0, min(MAX_DISP_MM, disp))
...
class CapstanPanel(tk.Frame):
    ...
    def _build(self):
        ...
        tk.Label(hf, text="  -  signed motor command: + pulls, - releases",
        ...
        for i in range(6):
            ...
            tk.Label(cell, text=f"M{i+1}", font=(FNT, 11, "bold"),
                     fg=col, bg=CARD).pack(pady=(2, 0))

            cv = tk.Canvas(cell, width=self.CANVAS_W, height=self.CANVAS_H, bg=CARD, highlightthickness=0)
            cv.pack(padx=2, pady=1)
            self._canvases.append(cv)

            dl = tk.Label(cell, text="+0.0", font=(FNT, 10, "bold"),
                          fg=col, bg=CARD, anchor="center")
            dl.pack()
            self._disp_lbls.append(dl)

            vl = tk.Label(cell, text="0 mm/s", font=(FNT, 8),
                          fg=MUTED, bg=CARD)
            vl.pack(pady=(0, 2))
    ...
    def _draw(self, i: int):
        ...
        # Displacement bar (bottom strip) - signed: center is home.
        d     = self._disps[i]
        ratio = max(-1.0, min(1.0, d / MAX_DISP_MM))
        mid_x = w // 2
        cv.create_rectangle(0, h - 11, w, h, fill=INPUT, outline="")
        cv.create_line(mid_x, h - 11, mid_x, h, fill=BORDER, width=1)
        bar_col = col if d >= 0 else BLUE
        if abs(ratio) > 0.001:
            bar_x = mid_x + int((w // 2 - 2) * ratio)
            cv.create_rectangle(min(mid_x, bar_x), h - 11,
                                max(mid_x, bar_x), h,
                                fill=bar_col, outline="")
        cv.create_text(w // 2, h - 5, text=f"{d:+.1f}mm",
...
class ContinuumVisualizer(tk.Frame):
    ...
    ────────────
      ...
      CENTRE — TOP VIEW   : forward/back projection + base-plate hexagon +
                            internal tendon paths through disk holes
                            (line width ∝ tension), zoom scroll
    ...
    # Tendon hole layout on the backbone cross-section.
    # Motors are in 3 antagonistic pairs; each pair straddles the backbone.
    # Top-view tendons are drawn as internal offset paths, not external cables.
    ...
    def _build(self):
        ...
        tk.Label(hf, text="view controls",
                 font=(FNT, 9), fg=DIM, bg=BG).pack(side="right")

        # Three canvases row
        views = tk.PanedWindow(self, orient=tk.HORIZONTAL, bg=BORDER, bd=0,
        ...
        views.add(sc, minsize=220, width=330)
        tk.Label(sc, text=" SIDE VIEW  (bending plane  x–y)",
                 font=(FNT, 9, "bold"), fg=BLUE, bg=PANEL).pack(fill="x")
        side_body = tk.Frame(sc, bg=CBKG)
        side_body.pack(fill="both", expand=True)
        self._cv_side = tk.Canvas(side_body, bg=CBKG, highlightthickness=0)
        side_scroll = ttk.Scrollbar(side_body, orient="vertical",
                                    command=self._cv_side.yview)
        self._cv_side.configure(yscrollcommand=side_scroll.set)
        side_scroll.pack(side="right", fill="y")
        self._cv_side.pack(side="left", fill="both", expand=True)
        ...
        views.add(tc, minsize=220, width=330)
        tk.Label(tc, text=" TOP VIEW  (base plate + internal tendon routing)",
                 font=(FNT, 9, "bold"), fg=ORANGE, bg=PANEL).pack(fill="x")
        top_body = tk.Frame(tc, bg=CBKG)
        top_body.pack(fill="both", expand=True)
        self._cv_top = tk.Canvas(top_body, bg=CBKG, highlightthickness=0)
        top_scroll = ttk.Scrollbar(top_body, orient="vertical",
                                   command=self._cv_top.yview)
        self._cv_top.configure(yscrollcommand=top_scroll.set)
        top_scroll.pack(side="right", fill="y")
        self._cv_top.pack(side="left", fill="both", expand=True)
        ...
        views.add(ic, minsize=240, width=380)
        ih = tk.Frame(ic, bg=PANEL)
        ih.pack(fill="x")
        tk.Label(ih, text=" ISOMETRIC VIEW",
        ...
        for label, yaw, pitch in [
            ...
            tk.Button(ih, text=label, font=(FNT, 8, "bold"),
        ...
        self._iso_zoom_lbl = tk.Label(ih, text="100%", font=(FNT, 8),
                                      fg=DIM, bg=PANEL, width=5)
        self._iso_zoom_lbl.pack(side="right", padx=(4, 2))
        iso_body = tk.Frame(ic, bg=CBKG)
        iso_body.pack(fill="both", expand=True)
        self._cv_iso = tk.Canvas(iso_body, bg=CBKG, highlightthickness=0)
        ...
        pc.pack(fill="x", pady=(3, 0))
        tk.Label(pc, text=" END-EFFECTOR", font=(FNT, 9, "bold"),
                 fg=ORANGE, bg=PANEL).pack(side="left", padx=(0, 8), fill="y")
        ...
        for key, label, unit, col in pose_entries:
            ...
            pf.pack(side="left", padx=6, pady=3)
        ...
        tk.Label(hc, text=" MOTOR LOAD HEATMAP  (blue=slack  ·  red=full pull  ·  width ∝ displacement)",
    ...
    @staticmethod
    def _fit_scroll_region(cv, pad=18):
        bbox = cv.bbox("all")
        if bbox:
            cv.configure(scrollregion=(bbox[0] - pad, bbox[1] - pad,
    ...
    def _draw_side(self):
        ...
        for s_mm, disk_seg, disk_kind in disk_layout_mm():
            ...
            fill_c = seg_col if is_end else ("#e5e7eb" if is_base else disk_col_fill)
            edge_c = seg_col if (is_end or is_base) else disk_col_edge
            lw_d   = 2 if is_end else 1
            ...
            pr = 2 if is_end else 1
            cv.create_oval(cx_d - pr, cy_d - pr, cx_d + pr, cy_d + pr,
                           fill=seg_col if is_end else "#6b7280", outline="")
    ...
    def _draw_top(self):
        ...
        def arc_normal(k, off_mm):
            ...
            ln = math.hypot(dx, dy) or 1.0
            nx, ny = -dy / ln, dx / ln
            scale = scl / TOTAL_LENGTH_MM
            return nx * off_mm * scale, -ny * off_mm * scale
        ...
        for s_mm, disk_seg, disk_kind in disk_layout_mm():
            ...
            k = min(int(round(s_mm / TOTAL_LENGTH_MM * (N - 1))), N - 1)
            dx, dy = px(*all_pts[k])
            r = 4 if disk_kind == "base" else 2
            outline = MUTED if disk_kind == "base" else "#64748b"
            cv.create_oval(dx - r, dy - r, dx + r, dy + r,
                           fill=CARD, outline=outline, width=1)
        ...
        tendon_offsets_mm = [
            TENDON_OFFSETS_MM[0], -TENDON_OFFSETS_MM[0],
            TENDON_OFFSETS_MM[1], -TENDON_OFFSETS_MM[1],
            TENDON_OFFSETS_MM[2], -TENDON_OFFSETS_MM[2],
        ]

        for i in range(6):
            col      = MOTOR_COLORS[i]
            seg_stop = self.TENDON_TERM[i]
            off_mm   = tendon_offsets_mm[i]
            d_val    = self._disps[i]
            active   = abs(d_val) > 0.5
            lw       = 2 if active else 1
            dash_    = () if d_val > 0.5 else ((2, 3) if d_val < -0.5 else (5, 3))
            ...
            for k in range(stop_k + 1):
                bx, by = px(*all_pts[k])
                ox, oy = arc_normal(k, off_mm)
                tx, ty = bx + ox, by + oy
                if base_x is None:
                    base_x, base_y = tx, ty
                if prev_x is not None:
                    cv.create_line(prev_x, prev_y, tx, ty,
                                   fill=col, width=lw, dash=dash_,
                                   capstyle=tk.ROUND, joinstyle=tk.ROUND)
                prev_x, prev_y = tx, ty

            if base_x is not None:
                cv.create_oval(base_x - 2, base_y - 2, base_x + 2, base_y + 2,
                               fill=col, outline="")
                cv.create_text(base_x, base_y + (8 if off_mm > 0 else -8),
                               text=f"M{i+1}", fill=col,
                               font=(FNT, 7, "bold"))
            if prev_x is not None:
                cv.create_oval(prev_x - 3, prev_y - 3,
                               prev_x + 3, prev_y + 3,
                               fill=col, outline=TEXT, width=1)
    ...
    def _draw_iso(self):
        ...
        scale = min(w, h) * 0.72 / TOTAL_LENGTH_MM * self._iso_zoom
        base_screen_x = w * 0.42
        base_screen_y = h * 0.78

        def project(p3):
            rx, ry, rz = rotate(p3)
            return (base_screen_x + rx * scale,
                    base_screen_y - ry * scale,
                    rz)
        ...
        def offset_point(k, radius_mm, angle_deg):
            x, y = all_pts[k]
            n, b = basis_at(k)
            a = math.radians(angle_deg)
            ca, sa = math.cos(a), math.sin(a)
            return (
                x + n[0] * radius_mm * ca + b[0] * radius_mm * sa,
                y + n[1] * radius_mm * ca + b[1] * radius_mm * sa,
                n[2] * radius_mm * ca + b[2] * radius_mm * sa,
            )
        ...
        for i in range(6):
            col = MOTOR_COLORS[i]
            seg_stop = self.TENDON_TERM[i]
            radius = TENDON_OFFSETS_MM[MOTOR_TO_SEG[i]]
            angle = self.TENDON_ANGLES_DEG[i]
            d_val = self._disps[i]
            active = abs(d_val) > 0.5
            lw = 2 if active else 1
            dash = () if d_val > 0.5 else ((2, 3) if d_val < -0.5 else (5, 3))
            ...
            prev = None
            for k in range(stop_k + 1):
                sx, sy_, _ = project(offset_point(k, radius, angle))
                if prev is not None:
                    cv.create_line(prev[0], prev[1], sx, sy_,
                                   fill=col, width=lw, dash=dash,
                                   capstyle=tk.ROUND, joinstyle=tk.ROUND)
                prev = (sx, sy_)
            if prev is not None:
                cv.create_oval(prev[0] - 3, prev[1] - 3,
                               prev[0] + 3, prev[1] + 3,
                               fill=col, outline=TEXT, width=1)
        ...
        for s_mm, disk_seg, disk_kind in disk_layout_mm():
            ...
            for q in range(24):
                ...
                p3 = (
                    cx0 + n[0] * rr * math.cos(a) + b[0] * rr * math.sin(a),
                    cy0 + n[1] * rr * math.cos(a) + b[1] * rr * math.sin(a),
                    n[2] * rr * math.cos(a) + b[2] * rr * math.sin(a),
                )
                px, py, dep = project(p3)
                ring += [px, py]
                depths.append(dep)
        ...
        for _, disk_kind, disk_seg, ring, k in sorted(disk_items, key=lambda item: item[0]):
            ...
            is_base = disk_kind == "base"
            seg_col = SEG_COLORS[disk_seg] if disk_seg >= 0 else MUTED
            fill = "#f8fafc" if not is_end else "#fff7ed"
            if is_base:
                fill = "#e5e7eb"
    ...
    def _draw_heat(self):
        ...
        for i in range(6):
            ...
            # Normalise 0 … MAX → 0 (slack/blue) … 1 (full pull/red)
            norm = max(0.0, min(1.0, d / MAX_DISP_MM))

            # Blue (slack) → dark → red (full pull)
            rr = int(0x18 + norm * (0xe8 - 0x18))
            gg = int(0x38 * (1 - norm))
            bb = int(0xd8 * (1 - norm) + 0x18)
...
class ManualJogPanel(tk.Frame):
    ...
    def __init__(self, parent, get_robot, get_disps, apply_disp_fn, log_fn, home_fn=None, **kw):
        super().__init__(parent, bg=BG, **kw)
    ...
    def _build(self):
        ...
        specs = [("", 4), ("Motor", 7), ("Displacement bar", 24),
                 ("Live mm", 9), ("Jog+", 7), ("Step+", 7),
                 ("Pull mm", 10), ("Speed mm/s", 11)]
        for t, w in specs:
            tk.Label(hdr, text=t, font=(FNT, 10, "bold"), fg=MUTED, bg=PANEL,
                     width=w, anchor="w").pack(side="left", padx=2, pady=3)
        tk.Label(hdr,
                 text="  command any motor with a positive pull amount; its partner releases equally",
                 font=(FNT, 9), fg=DIM, bg=PANEL).pack(side="left", padx=8)
    ...
    def _build_row(self, i: int):
        ...
        bar_fill = c.create_rectangle(2, 9, 2, 19, fill=col, outline="")
    ...
    def _step_jog(self, i):
        disps = self._get_disps()
        nd = max(-MAX_DISP_MM, min(MAX_DISP_MM, disps[i] + self._get_step()))
        self._apply_disp(i, nd, self._get_speed(i))
        self._disp_vars[i].set(f"{self._get_disps()[i]:.2f}")
    ...
    def _zero_all(self):
        # Delegate to GUI-level home which zeros all 6 simultaneously,
        # bypassing per-motor delta cascades.
        if self._home_fn:
            self._home_fn()
    ...
    def update_disp(self, idx1: int, disp: float):
        i = idx1 - 1
        # Bar — zero-based, positive only
        c  = self._bar_canvases[i]
        bw = 155
        ratio = max(-1.0, min(1.0, disp / MAX_DISP_MM))
        mid_x = bw // 2
        fill_x = mid_x + int((bw // 2 - 4) * ratio)
        col = MOTOR_COLORS[i]
        c.coords(self._bar_fills[i], min(mid_x, fill_x), 9, max(mid_x, fill_x), 19)
        c.itemconfig(self._bar_fills[i], fill=col if disp >= 0 else BLUE)
        c.itemconfig(self._bar_lbls[i], text=f"{disp:+.1f}")
        self._live_lbls[i].config(text=f"{disp:+.1f}")
        self._disp_vars[i].set(f"{max(0.0, disp):.2f}")
...
class CsvStatePanel(tk.Frame):
    ...
    def _build(self):
        ...
        self._options_anchor = tk.Frame(ctrl, bg=CARD, width=1)
        self._options_anchor.pack(side="left")
    ...
    def _enable_run(self):
        ...
        if self._body_frame and not self._body_frame.winfo_manager():
            self._body_frame.pack(fill="x", pady=(0, 4))

    def _show_idle_controls(self):
        self.btn_pause.pack_forget()
        self.btn_abort.pack_forget()
        if not self.btn_run.winfo_manager():
            self.btn_run.pack(side="left", padx=(0, 4), before=self._options_anchor)
        if not self.btn_step.winfo_manager():
            self.btn_step.pack(side="left", padx=(0, 4), before=self._options_anchor)

    def _show_running_controls(self):
        self.btn_run.pack_forget()
        self.btn_step.pack_forget()
        if not self.btn_pause.winfo_manager():
            self.btn_pause.pack(side="left", padx=(0, 4), before=self._options_anchor)
        if not self.btn_abort.winfo_manager():
            self.btn_abort.pack(side="left", padx=(0, 14), before=self._options_anchor)
...
class ContinuumGUI(tk.Tk):
    ...
    def _build(self):
        ...
        # ── Body ──────────────────────────────────────────────────────────────
        body = tk.PanedWindow(self, orient=tk.HORIZONTAL, bg=BORDER, bd=0,
                              sashwidth=8, sashrelief=tk.RAISED,
                              opaqueresize=True)
        body.pack(fill="both", expand=True)

        # Left: tab panels
        left_outer = tk.Frame(body, bg=BG, width=650)
        body.add(left_outer, minsize=360, width=650)
        ...
        def _scroll_router(e):
            viz = self._cont_viz if hasattr(self, "_cont_viz") else None
            if viz is not None and e.widget in (viz._cv_side, viz._cv_top, viz._cv_iso):
                return
            if vs.get() != (0.0, 1.0):
                lc.yview_scroll(int(-1 * (e.delta / 120)), "units")
        lc.bind_all("<MouseWheel>", _scroll_router)
        self._lc = lc

        # Tab container frame (single canvas window, always visible)
        self._tab_container = tk.Frame(lc, bg=BG)
        self._tab_win = lc.create_window((0, 0), window=self._tab_container, anchor="nw")
        self._tab_container.bind("<Configure>",
        ...
        body.add(right, minsize=520, width=950)
    ...
    def _apply_single_disp(self, motor_idx: int, disp_mm: float, speed_mms: float):
        # Each antagonistic pair is one signed actuation coordinate. Jogging a
        # released motor in the positive direction should pass through neutral
        # before that motor becomes the puller.
        partner_idx = PAIR_MATES[motor_idx]
        disp_mm = max(-MAX_DISP_MM, min(MAX_DISP_MM, disp_mm))
        pair_targets = ((motor_idx, disp_mm), (partner_idx, -disp_mm))

        for idx, val in pair_targets:
            self._disps[idx]  = val
            self._speeds[idx] = speed_mms
            self._capstan_panel.update(idx + 1, val, speed_mms)
            self._manual_panel.update_disp(idx + 1, val)
        ...
        if self.robot and HAS_ROBOT:
            for idx, val in pair_targets:
                try:
                    ...
                    cfg = DEFAULT_JOINT_CONFIG[idx]
                    threading.Thread(
                        target=self.robot.motors[cfg.motor_id].set_position,
                        kwargs={"position_deg": mm_to_deg(val, idx),
                                "max_speed_dps": mms_to_dps(speed_mms, idx),
                                "wait": False},
                        daemon=True).start()
                except Exception as e:
                    self._log(f"HW error M{idx+1}: {e}", "err")

    def _apply_state(self, state: TendonState):
        targets = [max(0.0, min(MAX_DISP_MM, m[0])) for m in state.motors]
        speeds  = [m[1] for m in state.motors]
        for pair_start in (0, 2, 4):
            a = pair_start
            b = pair_start + 1
            amount = max(targets[a], targets[b])
            pair_speed = max(speeds[a], speeds[b])
            if amount <= 0.0:
                targets[a] = targets[b] = 0.0
            elif targets[a] >= targets[b]:
                targets[a], targets[b] = amount, -amount
            else:
                targets[a], targets[b] = -amount, amount
            speeds[a] = speeds[b] = pair_speed
        for i in range(6):
    ...
    def _home_all(self):
        # Set all motors to 0 simultaneously — bypass antagonistic delta logic
        # since all motors going to 0 at once is a valid neutral state.
        for i in range(6):
            self._disps[i]  = 0.0
            self._speeds[i] = 20.0
            self._capstan_panel.update(i + 1, 0.0, 20.0)
            self._manual_panel.update_disp(i + 1, 0.0)
        self._cont_viz.set_displacements(self._disps)
        if self.robot and HAS_ROBOT:
            for i in range(6):
                try:
                    ...
                    threading.Thread(
                        target=self.robot.motors[cfg.motor_id].set_position,
                        kwargs={"position_deg": 0.0, "max_speed_dps": mms_to_dps(20.0, i),
                                "wait": False},
                        daemon=True).start()
                except Exception as e:
                    self._log(f"HW error M{i+1}: {e}", "err")
