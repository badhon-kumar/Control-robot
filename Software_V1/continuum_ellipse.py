...
def compute_pcc_kinematics_mm(disps_mm: List[float]) -> dict:
    ...
    for seg_i in range(3):
        ...
        ri  = R_TENDON[seg_i] * 1000.0
        ki  = (dl[seg_i] * 1000.0 / ri) / Li if ri > 0 else 0.0
...
class ContinuumVisualizer(tk.Frame):
    ...
    def _draw_side(self):
        ...
        for anch_mm, name in zip([90, 180, 270], ["S1/S2", "S2/S3", "tip"]):
            idx = min(int(round(anch_mm / (TOTAL_LEN_MM/120))), 120)
            ax2, ay2 = px(*all_pts[idx])
            cv.create_oval(ax2-4, ay2-4, ax2+4, ay2+4,
                           fill=CARD, outline=MUTED, width=1)
    ...
    def _draw_top(self):
        """
        TOP VIEW — X-Z plane (looking down the Y-axis from above).
        X = along arm length (0 mm at base → ~270 mm at tip)
        Z = out-of-plane  (always = 0 mm for this planar robot)
        The arm always appears as a perfectly straight horizontal line at Z=0.
        """
        cv = self._cv_top
        w, h = cv.winfo_width(), cv.winfo_height()
        if w < 30 or h < 30: return
        ...
        mg_l, mg_r, mg_t, mg_b = 42, 12, 20, 28

        plot_w = w - mg_l - mg_r
        plot_h = h - mg_t - mg_b
        scl_x  = plot_w / (X_HI - X_LO) * self._top_zoom
        scl_z  = plot_h / (Z_HI - Z_LO) * self._top_zoom
        scl    = min(scl_x, scl_z)

        ox = mg_l + int((0 - X_LO) * scl)     # X=0 pixel
        oz = mg_t + int((Z_HI - 0) * scl)      # Z=0 pixel (centre)

        def px(xmm, zmm=0.0):
            return ox + xmm * scl, oz - zmm * scl
        ...
        for zv in range(-60, 61, 30):
            ...
            if 0 < gz < h:
                lw = 2 if zv == 0 else 1
                cv.create_line(mg_l, gz, w-mg_r, gz,
                               fill="#c8d8e8" if zv == 0 else "#e2e8f0", width=lw)

        # ── Ellipse X-range highlight ─────────────────────────────────────────
        ex0, _ = px(220); ex1, _ = px(260)
        _, ez0  = px(0, 20); _, ez1 = px(0, -20)
        cv.create_rectangle(ex0, ez0, ex1, ez1,
                            fill="#e8f5e9", outline="#a5d6a7", width=1, dash=(3,3))
        cv.create_text((ex0+ex1)//2, ez0-2, text="ellipse X-range",
                       fill="#4caf50", font=(FNT, 7), anchor="s")

        # ── Z=0 line (arm rest axis) ──────────────────────────────────────────
        zx0, zz = px(X_LO); zx1, _ = px(X_HI)
        cv.create_line(zx0, zz, zx1, zz, fill="#90a4ae", width=1, dash=(5,3))
        ...
        cv.create_line(ax0, ay, ax1, ay, fill=MUTED, width=1)
        # Z axis
        zax, zay0 = px(0, Z_HI); _, zay1 = px(0, Z_LO)
        cv.create_line(zax, zay0, zax, zay1, fill=MUTED, width=1)
        ...
        for zv in range(-60, 61, 30):
            _, gz = px(0, zv)
            if mg_t < gz < h - mg_b:
                cv.create_line(zax-3, gz, zax+3, gz, fill=MUTED, width=1)
                if zv != 0:
                    cv.create_text(zax-5, gz, text=str(zv), fill=DIM,
                                   font=(FNT, 7), anchor="e")

        cv.create_text(w - mg_r - 2, ay - 4, text="X (mm) →",
                       fill=MUTED, font=(FNT, 8, "bold"), anchor="e")
        cv.create_text(zax + 3, mg_t + 2, text="↑ Z (mm)",
                       fill=MUTED, font=(FNT, 8, "bold"), anchor="nw")

        # ── Z=0 label ─────────────────────────────────────────────────────────
        cv.create_text(mg_l + 2, oz - 4, text="Z = 0",
                       fill="#607d8b", font=(FNT, 8, "italic"), anchor="sw")

        # ── Base plate ────────────────────────────────────────────────────────
        plate_r = 14
        cv.create_oval(ox - plate_r, oz - plate_r, ox + plate_r, oz + plate_r,
...
class TrajectoryControlPanel(tk.Frame):
    ...
    def _build(self):
        ...
        tk.Frame(lf, width=380, height=0, bg=BG).pack() # Enforce width without clipping height
        ...
        tk.Checkbutton(cc, text="Enable Kalman compensation",
                       variable=self._kalman_var, font=(FNT,12,"bold"),
                       fg=GREEN, bg=CARD, selectcolor=INPUT, activebackground=CARD).grid(
            row=len(params), column=0, columnspan=2, sticky="w", padx=8, pady=5)
        ...
        # Control buttons — bigger, full-width, use available vertical space
        bf = tk.Frame(lf, bg=BG)
        bf.pack(fill="x", pady=(10, 4), padx=2)
        self.btn_start = tk.Button(bf, text="▶   START TRACKING", font=(FNT,16,"bold"),
                                   bg=GREEN, fg=BG, relief="flat", cursor="hand2",
                                   padx=24, pady=16, command=self._start)
        self.btn_start.pack(fill="x", pady=(0, 6))
        self.btn_stop  = tk.Button(bf, text="■   STOP",  font=(FNT,16,"bold"),
                                   bg=RED, fg=TEXT, relief="flat", cursor="hand2",
                                   padx=24, pady=16, command=self._stop, state="disabled")
        self.btn_stop.pack(fill="x", pady=(0, 6))
        tk.Button(bf, text="↺  RESET PLOTS", font=(FNT,14,"bold"),
                  bg=PANEL, fg=MUTED, relief="flat", cursor="hand2",
                  padx=18, pady=14, command=self._reset).pack(fill="x")
        ...
        qa = tk.LabelFrame(lf, text="  Quick Actions  ",
        ...
        tp_lbl = tk.Label(rf, text="END-EFFECTOR TRAJECTORY  (mm)  —  Fig. 6  Zhai et al. 2025",
                          font=(FNT,12,"bold"), fg=BLUE, bg=PANEL)
        tp_lbl.pack(fill="x")
        self._cv_traj = tk.Canvas(rf, bg=CBKG, highlightthickness=1,
        ...
        self._cv_err = tk.Canvas(rf, bg=CBKG, highlightthickness=1,
                                 highlightbackground=BORDER,
...
class ContinuumGUI(tk.Tk):
    ...
    def _build(self):
        ...
        left_outer = tk.Frame(body, bg=BG, width=920)
        ...
        # Scrollable canvas for trajectory panel
        ...
        def _scroll_router(e):
            # If the widget under the cursor belongs to the continuum visualizer
            # canvases, let those bindings handle zoom instead.
            w_under = e.widget
            viz = self._cont_viz if hasattr(self, '_cont_viz') else None
            if viz is not None:
                if w_under is viz._cv_side or w_under is viz._cv_top:
                    return  # let canvas-level bindings handle it
