...
────────
  ...
  ③ TRAJECTORY   — closed-loop PCC + Kalman Jacobian compensation tracking

Run:
    python continuum_gui_v2.py             ← simulation
    python continuum_gui_v2.py --real --port COM3
"""
...
MAX_DELTA_L       = 0.060                    # max tendon differential input (m)
...
# ══════════════════════════════════════════════════════════════════════════════
# KALMAN JACOBIAN COMPENSATED PCC CONTROLLER
# ══════════════════════════════════════════════════════════════════════════════

class KalmanJacobianController:
    def __init__(self, params: dict = None):
        self.params = dict(params or {})
        self.L = np.array(self.params.get("L", [0.09, 0.09, 0.09]), dtype=float)
        self.r = np.array(self.params.get("r", [0.005, 0.0035, 0.002]), dtype=float)
        self.alpha = float(self.params.get("alpha", 1.0))
        self.beta = float(self.params.get("beta", 0.5))
        self.gamma = float(self.params.get("gamma", 0.5))
        self.sigma_p = float(self.params.get("sigma_p", 0.35))
        self.sigma_psi = float(self.params.get("sigma_psi", 2.5))
        self.use_kalman = bool(self.params.get("use_kalman", True))
        self.h = float(self.params.get("fd_h", 1e-6))
        self.Q = np.eye(9)
        self.I9 = np.eye(9)
        self.I3 = np.eye(3)
        self.diagnostics = {}
        self.last_kalman_gain_norm = 0.0
        self.reset()
    ...
    def reset(self):
        ...
        self.pose_prev = self.forward_kinematics(self.u_prev)
        self.cJ_prev = self.model_jacobian(self.u_prev)
        self.diagnostics = {}
        self.last_kalman_gain_norm = 0.0

    def pcc_angles(self, u: np.ndarray) -> np.ndarray:
        u = np.asarray(u, dtype=float).reshape(3)
        th1 = u[0] / self.r[0]
        th2 = (u[1] - self.r[1] * th1) / self.r[0]
        th3 = (u[2] - self.r[2] * th1 - self.r[1] * th2) / self.r[0]
        return np.array([th1, th2, th3], dtype=float)

    def forward_kinematics(self, u: np.ndarray) -> np.ndarray:
        theta = self.pcc_angles(u)
        x = 0.0
        y = 0.0
        phi = 0.0
        for i in range(3):
            ...
            if abs(th) < 1e-6:
                ...
                x += (L / th) * (math.sin(phi + th) - math.sin(phi))
                y += (L / th) * (math.cos(phi) - math.cos(phi + th))
            phi += th
        return np.array([x, y, phi], dtype=float)

    def model_jacobian(self, u: np.ndarray) -> np.ndarray:
        u = np.asarray(u, dtype=float).reshape(3)
        mJ = np.zeros((3, 3))
        for j in range(3):
            u_plus = u.copy()
            ...
            u_plus[j] += self.h
            u_minus[j] -= self.h
            mJ[:, j] = (self.forward_kinematics(u_plus) -
                        self.forward_kinematics(u_minus)) / (2.0 * self.h)
        return mJ

    @staticmethod
    def _measurement_matrix(du: np.ndarray) -> np.ndarray:
        H = np.zeros((3, 9))
        H[0, 0:3] = du
        H[1, 3:6] = du
        H[2, 6:9] = du
    ...
    def kalman_update(self, u_curr, u_prev, pose_curr, pose_prev, mJ):
        ...
        pose_prev = np.asarray(pose_prev, dtype=float).reshape(3)
        xi_prior = self.gamma * self.xi
        P_prior = (self.gamma ** 2) * self.P + self.Q
        du = u_curr - u_prev
        H = self._measurement_matrix(du)
        z = pose_curr - pose_prev
        R = 0.5 * float(np.linalg.norm(du) ** 2) * np.eye(3)
        residual = z - H @ (mJ.reshape(9) + xi_prior)
        S = H @ P_prior @ H.T + R + 1e-12 * np.eye(3)
        K = P_prior @ H.T @ np.linalg.pinv(S)
        self.last_kalman_gain_norm = float(np.linalg.norm(K))
        self.xi = xi_prior + K @ residual
        self.P = (self.I9 - K @ H) @ P_prior
        self.P = 0.5 * (self.P + self.P.T)
        return mJ + self.xi.reshape((3, 3))

    def apply_constraints(self, eJ: np.ndarray) -> np.ndarray:
        eJ = np.asarray(eJ, dtype=float).reshape((3, 3))
        cJ = np.zeros((3, 3))
        dp = eJ[0:2, :] - self.cJ_prev[0:2, :]
        np_norm = float(np.linalg.norm(dp, ord="fro"))
        if np_norm <= self.sigma_p or np_norm < 1e-12:
            cJ[0:2, :] = eJ[0:2, :]
        else:
            cJ[0:2, :] = self.cJ_prev[0:2, :] + self.sigma_p * dp / np_norm
        dpsi = eJ[2:3, :] - self.cJ_prev[2:3, :]
        npsi = float(np.linalg.norm(dpsi, ord="fro"))
        if npsi <= self.sigma_psi or npsi < 1e-12:
            cJ[2:3, :] = eJ[2:3, :]
        else:
            cJ[2:3, :] = self.cJ_prev[2:3, :] + self.sigma_psi * dpsi / npsi
        self.cJ_prev = cJ.copy()
        return cJ

    def compute_control(self, u_curr, pose_ref, pose_curr) -> np.ndarray:
        u_curr = np.asarray(u_curr, dtype=float).reshape(3)
        pose_ref = np.asarray(pose_ref, dtype=float).reshape(3)
        pose_curr = np.asarray(pose_curr, dtype=float).reshape(3)
        mJ = self.model_jacobian(u_curr)
        if self.use_kalman:
            eJ = self.kalman_update(u_curr, self.u_prev, pose_curr, self.pose_prev, mJ)
        else:
            eJ = mJ.copy()
        cJ = self.apply_constraints(eJ)
        cJ_dag = np.linalg.pinv(cJ.T @ cJ + self.alpha * self.I3) @ cJ.T
        err = pose_ref - pose_curr
        u_next = u_curr + self.beta * (cJ_dag @ err)
        self.diagnostics = {
            "mJ": mJ.copy(),
            "eJ": eJ.copy(),
            "cJ": cJ.copy(),
            "error": err.copy(),
            "kalman_gain_norm": float(self.last_kalman_gain_norm),
        }
...
class TrajectoryControlPanel(tk.Frame):
    TRAJ_NAMES = ["Traj. 1 - Ellipse", "Traj. 2 - Attitude", "Traj. 3 - Combined", "Figure-8", "Line", "Custom"]

    def __init__(self, parent, apply_disps_fn, log_fn, **kw):
        super().__init__(parent, bg=BG, **kw)
        self._apply_disps = apply_disps_fn
        self._log = log_fn
        self._running = False
        self._thread = None
        ...
        self._traj_var = tk.StringVar(value=self.TRAJ_NAMES[0])
        self._use_kalman_var = tk.BooleanVar(value=True)
        self._noise_var = tk.BooleanVar(value=True)
        self._show_jac_var = tk.BooleanVar(value=False)
        self._vars = {
            "gamma": tk.StringVar(value="0.5"),
            "beta": tk.StringVar(value="0.5"),
            "alpha": tk.StringVar(value="1.0"),
            "sigma_p": tk.StringVar(value="0.35"),
            "sigma_psi": tk.StringVar(value="2.5"),
            "period": tk.StringVar(value="40.0"),
            "rate": tk.StringVar(value="20.0"),
            "noise": tk.StringVar(value="0.0005"),
        }
        self._metric_lbls = {}
        self._build()
    ...
    def _build(self):
        ...
        left = tk.Frame(root, bg=CARD, highlightbackground=BORDER, highlightthickness=1, width=250)
        left.pack(side="left", fill="y", padx=(0, 4))
        left.pack_propagate(False)
        right = tk.Frame(root, bg=BG)
        right.pack(side="left", fill="both", expand=True)
        tk.Label(left, text="TRAJECTORY CONTROL", font=(FNT_H, 12, "bold"),
                 fg=ACCENT, bg=CARD).pack(anchor="w", padx=10, pady=(8, 4))
        ttk.Combobox(left, textvariable=self._traj_var, values=self.TRAJ_NAMES,
                     state="readonly", width=25).pack(anchor="w", padx=10, pady=4)

        for key, label in [
            ("gamma", "γ fading"), ("beta", "β gain"), ("alpha", "α damping"),
            ("sigma_p", "σp limit"), ("sigma_psi", "σψ limit"),
            ("period", "T period s"), ("rate", "Rate Hz"), ("noise", "Noise σ m"),
        ]:
            row = tk.Frame(left, bg=CARD)
            row.pack(fill="x", padx=10, pady=2)
            tk.Label(row, text=label, width=13, anchor="w", font=(FNT, 9),
                     fg=MUTED, bg=CARD).pack(side="left")
            tk.Entry(row, textvariable=self._vars[key], width=9, font=(FNT, 10),
                     bg=INPUT, fg=TEXT, relief="flat",
                     highlightbackground=BORDER, highlightthickness=1).pack(side="right")

        tk.Checkbutton(left, text="Use Kalman compensation", variable=self._use_kalman_var,
                       font=(FNT, 9), bg=CARD, fg=TEXT, selectcolor=INPUT,
                       activebackground=CARD).pack(anchor="w", padx=8, pady=(6, 0))
        tk.Checkbutton(left, text="Add measurement noise", variable=self._noise_var,
                       font=(FNT, 9), bg=CARD, fg=TEXT, selectcolor=INPUT,
                       activebackground=CARD).pack(anchor="w", padx=8)
        tk.Checkbutton(left, text="Show Jacobian trend", variable=self._show_jac_var,
                       font=(FNT, 9), bg=CARD, fg=TEXT, selectcolor=INPUT,
                       activebackground=CARD).pack(anchor="w", padx=8)

        btns = tk.Frame(left, bg=CARD)
        btns.pack(fill="x", padx=10, pady=8)
        tk.Button(btns, text="START", font=(FNT, 10, "bold"), bg=GREEN, fg=BG,
                  relief="flat", command=self.start).pack(side="left", expand=True, fill="x", padx=1)
        tk.Button(btns, text="STOP", font=(FNT, 10, "bold"), bg=RED, fg=TEXT,
                  relief="flat", command=self.stop).pack(side="left", expand=True, fill="x", padx=1)
        tk.Button(btns, text="RESET", font=(FNT, 10, "bold"), bg=BLUE, fg=BG,
                  relief="flat", command=self.reset).pack(side="left", expand=True, fill="x", padx=1)

        tk.Label(left, textvariable=self._status_var, font=(FNT, 10, "bold"),
                 fg=BLUE, bg=CARD).pack(anchor="w", padx=10, pady=(2, 6))
        tk.Label(left, text="Custom: select Custom, then click plot to add waypoints.",
                 font=(FNT, 8), fg=DIM, bg=CARD, wraplength=220,
                 justify="left").pack(anchor="w", padx=10, pady=(0, 6))

        metrics = tk.Frame(left, bg=INPUT, highlightbackground=BORDER, highlightthickness=1)
        metrics.pack(fill="x", padx=10, pady=4)
        for key in ["RMSE x", "RMSE y", "RMSE ψ", "MAE x", "MAE y", "MAE ψ", "Current |e|"]:
            row = tk.Frame(metrics, bg=INPUT)
            row.pack(fill="x", padx=5, pady=1)
            tk.Label(row, text=key, width=11, anchor="w", font=(FNT, 8),
                     fg=MUTED, bg=INPUT).pack(side="left")
            lbl = tk.Label(row, text="--", anchor="e", font=(FNT, 8, "bold"),
                           fg=TEXT, bg=INPUT)
            lbl.pack(side="right")
            self._metric_lbls[key] = lbl

        self._cv = tk.Canvas(right, height=400, bg=CBKG, highlightthickness=0)
        self._cv.pack(fill="both", expand=True)
        self._cv.bind("<Configure>", lambda e: self._draw())
        self._cv.bind("<Button-1>", self._on_canvas_click)

    def _num(self, key, default, lo=None, hi=None):
        try:
    ...
    def _params(self):
        return {
            "L": [0.09, 0.09, 0.09],
            "r": [0.005, 0.0035, 0.002],
            "gamma": self._num("gamma", 0.5, 0.0, 1.0),
            "beta": self._num("beta", 0.5, 0.0, 5.0),
            "alpha": self._num("alpha", 1.0, 1e-9, 100.0),
            "sigma_p": self._num("sigma_p", 0.35, 0.0, 100.0),
            "sigma_psi": self._num("sigma_psi", 2.5, 0.0, 100.0),
            "use_kalman": self._use_kalman_var.get(),
        }

    def start(self):
        if self._running:
            return
        self.controller = KalmanJacobianController(self._params())
        self._hist_ref.clear()
        self._hist_actual.clear()
        self._hist_err.clear()
        self._last_u = np.zeros(3)
        self._running = True
        self._status_var.set("Running")
        self._log(f"[TRAJ] Started {self._traj_var.get()}", "ok")
    ...
    def reset(self):
        ...
        self._apply_disps([0.0] * 6, [20.0] * 6)
        self._status_var.set("Idle")
        self._draw()

    def _trajectory(self, t):
        T = max(0.1, self._num("period", 40.0, 0.1, 10000.0))
        w = 2.0 * math.pi * (t % T) / T
        name = self._traj_var.get()
        if name.startswith("Traj. 1"):
            return np.array([0.240 + 0.020 * math.cos(w), 0.060 * math.sin(w), 0.0])
        if name.startswith("Traj. 2"):
            return np.array([0.240, 0.0, (math.pi / 6.0) * math.sin(w)])
        if name.startswith("Traj. 3"):
            return np.array([0.240 + 0.020 * math.cos(w), 0.060 * math.sin(w),
                             (math.pi / 6.0) * math.sin(w)])
        if name.startswith("Figure-8"):
            return np.array([0.240 + 0.020 * math.sin(w), 0.040 * math.sin(2.0 * w), 0.0])
        if name.startswith("Line"):
            return np.array([0.240, 0.060 * math.sin(w), 0.0])
        if len(self._custom_pts) >= 2:
            pts = self._custom_pts
            s = (t % T) / T * len(pts)
            i = int(math.floor(s)) % len(pts)
            a = s - math.floor(s)
            p0 = np.array([pts[i][0], pts[i][1], 0.0])
            p1 = np.array([pts[(i + 1) % len(pts)][0], pts[(i + 1) % len(pts)][1], 0.0])
            return (1.0 - a) * p0 + a * p1
        return np.array([0.240, 0.0, 0.0])

    @staticmethod
    def _u_to_motor_disps(u):
        out = [0.0] * 6
        for i, val in enumerate(np.asarray(u, dtype=float).reshape(3)):
            mm = abs(float(val)) * 1000.0
            if val >= 0:
                out[2 * i] = mm
                out[2 * i + 1] = 0.0
            else:
    ...
    def _control_loop(self):
        rate = self._num("rate", 20.0, 1.0, 100.0)
        dt = 1.0 / rate
        t = 0.0
        u_curr = self._last_u.copy()
        period = max(0.1, self._num("period", 40.0, 0.1, 10000.0))
        while self._running and t <= period:
            t_start = time.time()
            pose_ref = self._trajectory(t)
            pose_curr = self.controller.forward_kinematics(u_curr)
            if self._noise_var.get():
                sig = self._num("noise", 0.0005, 0.0, 0.1)
                pose_curr = pose_curr + np.random.normal(0.0, sig, 3)
            u_next = self.controller.compute_control(u_curr, pose_ref, pose_curr)
            u_next = np.clip(u_next, -MAX_DELTA_L, MAX_DELTA_L)
            motor_disps = self._u_to_motor_disps(u_next)
            self.after(0, self._update_gui, motor_disps, pose_curr, pose_ref, t, u_next)
            u_curr = u_next
            t += dt
    ...
    def _update_gui(self, motor_disps, pose_curr, pose_ref, t, u_next):
        self._last_u = np.asarray(u_next, dtype=float)
        self._apply_disps(motor_disps, [MAX_SPEED_MMS] * 6)
        err = np.asarray(pose_ref) - np.asarray(pose_curr)
        self._hist_ref.append(np.asarray(pose_ref))
        self._hist_actual.append(np.asarray(pose_curr))
    ...
    def _finish_run(self):
        if self._running:
            ...
            self._summary()
        elif self._status_var.get() == "Stopping":
            self._status_var.set("Stopped")

    def _refresh_metrics(self):
        if not self._hist_err:
            return
        e = np.vstack(self._hist_err)
        rmse = np.sqrt(np.mean(e ** 2, axis=0))
        mae = np.mean(np.abs(e), axis=0)
        cur = float(np.linalg.norm(e[-1, :2]))
        vals = {
            "RMSE x": f"{rmse[0]*1000:.2f} mm",
            "RMSE y": f"{rmse[1]*1000:.2f} mm",
            "RMSE ψ": f"{math.degrees(rmse[2]):.2f}°",
            "MAE x": f"{mae[0]*1000:.2f} mm",
            "MAE y": f"{mae[1]*1000:.2f} mm",
            "MAE ψ": f"{math.degrees(mae[2]):.2f}°",
            "Current |e|": f"{cur*1000:.2f} mm",
        }
        for k, v in vals.items():
            self._metric_lbls[k].config(text=v)
    ...
    def _summary(self):
        ...
        rmse = np.sqrt(np.mean(e ** 2, axis=0))
        mae = np.mean(np.abs(e), axis=0)
        self._log("[TRAJ] Summary  RMSE(x,y,ψ)="
                  f"{rmse[0]*1000:.2f}mm, {rmse[1]*1000:.2f}mm, {math.degrees(rmse[2]):.2f}deg  "
                  f"MAE(x,y,ψ)={mae[0]*1000:.2f}mm, {mae[1]*1000:.2f}mm, {math.degrees(mae[2]):.2f}deg",
                  "ok")

    def _on_canvas_click(self, event):
        if self._traj_var.get() != "Custom":
            return
        w, h = max(1, self._cv.winfo_width()), max(1, self._cv.winfo_height())
        left, top, right, bottom = 48, 18, w - 20, int(h * 0.66) - 18
        x = 0.18 + (event.x - left) / max(1, right - left) * 0.12
        y = 0.08 - (event.y - top) / max(1, bottom - top) * 0.16
        self._custom_pts.append((float(x), float(y)))
        self._draw()
    ...
    def _draw(self):
        ...
        cv.delete("all")
        plot_h = int(h * 0.66)
        left, top, right, bottom = 48, 18, w - 20, plot_h - 18
        cv.create_rectangle(left, top, right, bottom, outline=BORDER, fill=INPUT)
        cv.create_text(left, top - 4, text="Tip trajectory x-y (m)", fill=MUTED,
                       font=(FNT, 9), anchor="sw")

        def xy(p):
            x = left + (p[0] - 0.18) / 0.12 * (right - left)
            y = bottom - (p[1] + 0.08) / 0.16 * (bottom - top)
            return x, y

        T = max(0.1, self._num("period", 40.0, 0.1, 10000.0))
        ref_pts = []
        for i in range(161):
            ref_pts.extend(xy(self._trajectory(T * i / 160.0)))
        if len(ref_pts) >= 4:
            cv.create_line(*ref_pts, fill=BLUE, dash=(5, 4), width=2)
        if self._custom_pts:
            cps = []
            for p in self._custom_pts:
                cps.extend(xy((p[0], p[1], 0.0)))
            for px, py in zip(cps[0::2], cps[1::2]):
                cv.create_oval(px - 3, py - 3, px + 3, py + 3, fill=ORANGE, outline="")
            if len(cps) >= 4:
                cv.create_line(*cps, fill=ORANGE, dash=(2, 3))
        if len(self._hist_actual) >= 2:
            act_pts = []
            for p in self._hist_actual[-800:]:
                act_pts.extend(xy(p))
            cv.create_line(*act_pts, fill=GREEN, width=2)
        if self._hist_actual:
            ax, ay = xy(self._hist_actual[-1])
            rx, ry = xy(self._hist_ref[-1])
            cv.create_oval(ax - 4, ay - 4, ax + 4, ay + 4, fill=GREEN, outline=TEXT)
            cv.create_oval(rx - 4, ry - 4, rx + 4, ry + 4, fill=BLUE, outline=TEXT)

        e_top, e_bot = plot_h + 10, h - 18
        cv.create_rectangle(left, e_top, right, e_bot, outline=BORDER, fill=INPUT)
        cv.create_text(left, e_top - 4, text="Tracking error magnitude", fill=MUTED,
                       font=(FNT, 9), anchor="sw")
        if len(self._hist_err) >= 2:
            mags = [float(np.linalg.norm(e[:2])) * 1000.0 for e in self._hist_err[-800:]]
            mmax = max(1.0, max(mags))
            pts = []
            for i, val in enumerate(mags):
                x = left + i / max(1, len(mags) - 1) * (right - left)
                y = e_bot - val / mmax * (e_bot - e_top)
                pts.extend([x, y])
            cv.create_line(*pts, fill=RED, width=2)
            cv.create_text(right - 4, e_top + 4, text=f"{mmax:.1f} mm",
                           fill=DIM, font=(FNT, 8), anchor="ne")
...
class ContinuumGUI(tk.Tk):
    ...
    def _build(self):
        ...
        self._tab_traj_btn = tk.Button(
            tbar, text="  🎯  TRAJECTORY CONTROL  ",
            font=(FNT, 12, "bold"), bg=PANEL, fg=MUTED,
            relief="flat", cursor="hand2", padx=10, pady=6,
            command=self._show_traj)
        ...
        self._traj_panel = TrajectoryControlPanel(
            self._traj_frame,
            apply_disps_fn = self._apply_disps_quiet,
            log_fn         = self._log,
        )
    ...
    def _apply_disps_quiet(self, targets, speeds):
        targets = [max(0.0, min(MAX_DISP_MM, float(v))) for v in targets]
        speeds = [max(0.1, min(MAX_SPEED_MMS, float(v))) for v in speeds]
        for i in range(6):
