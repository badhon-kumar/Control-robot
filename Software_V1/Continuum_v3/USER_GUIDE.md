...
This guide explains the adjustable parameters in the continuum manipulator GUI, especially the ellipse and G-code trajectory tracking controls in `continuum_ellipse.py`.
...
The default ellipse tracking mode moves the manipulator tip along the reference path:
...
The G-code mode instead reads a `.gcode` file and converts simple `G0`/`G1` motion commands into the same kind of reference pose:
...
| G-code file         |               sample_square_path.gcode |
...
Currently supported trajectory types are:
...
For G-code, the tip follows the `X`, `Y`, and optional attitude values from the loaded file.
...
The G-code file contains a list of target moves for the robot tip.
...
When `Trajectory Type` is set to `G-code path`, the GUI loads this file and follows it as the reference path.

**How to use**

1. Select `G-code path` from the trajectory type box.
2. Choose a file from the G-code dropdown, or click `Browse File`.
   The dropdown automatically lists `.gcode`, `.nc`, and `.tap` files in the `Continuum_v3` folder.
   Use `Refresh` if you add a new G-code file while the GUI is already open.

   You can also keep the default:

   ```text
   sample_square_path.gcode
   ```
...
G3   = counterclockwise circular arc
I/J  = arc center offset for G2/G3
X/Y  = target tip position
A    = target tip attitude in degrees
F    = feed rate in mm/min
```

For a fuller beginner explanation, read:
...
Start with the provided sample file. Keep the path inside a reasonable workspace near:
...
For round paths, use `G2/G3` arcs. For ellipses or irregular curves, use many small `G1` line segments for now.
...
In `G-code path` mode, the path timing comes from the G-code feed rates, so `Period T` is not the main timing control.
...
In `G-code path` mode, it is the number of times the loaded G-code path is repeated.
