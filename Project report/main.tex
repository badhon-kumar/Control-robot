...
% ─── Bulk Caps: NOW between breaker output and fuse box ──────────────
\node[component, minimum width=1.8cm, minimum height=1.4cm, font=\scriptsize] (caps) at (-0.5, 0.8) {Bulk Cap\\12$\times$ 4700$\mu$F};
\draw[heavyposline] (breaker.east) -- (caps.west);          % Pos: breaker → caps
%\draw[heavynegline] (-5.8, 2.5) -| (caps.south);           % Neg: busbar → caps (bottom)

% ─── Fuse Box ────────────────────────────────────────────────────────
\node[component] (fusebox) at (3.0, 0.8) {6-Way Fuse Box\\(20A per circuit)};
\draw[heavyposline] (caps.east) -- (fusebox.west);          % Pos: caps → fuse box

% ─── Motors & External Routing ───────────────────────────────────────
...
\draw[heavyposline] (fusebox.south) -- (3.0, -1.0) -- (-5.2, -1.0);
\draw[heavyposline] (3.0, -1.0) -- (4.8, -1.0);
\foreach \x/\i in {-5.0/1, -3.0/2, -1.0/3, 1.0/4, 3.0/5, 5.0/6} {
...
\end{tikzpicture}
\caption{System-level electrical routing schematic. The architecture illustrates the parallel 48V DC power generation, centralized busbar distribution, and hierarchical circuit protection for the 6-axis actuator network. Bulk capacitors are placed after the 150A main breaker for protected decoupling.}
\end{figure}
